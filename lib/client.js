...
Client.prototype._buildForward = function() {
  var that = this;
  var uid = crypto.randomBytes(32).toString('base64');
  this.forward = function(topic, payload, options, subTopic, initialQoS, cb) {
    if (options && options.mosca && options.mosca.trail) {
      if (options.mosca.trail[uid]) {
        return;
      }
      options.mosca.trail[uid] = true;
    }
...
Client.prototype.handleAuthorizePublish = function(err, success, packet) {
  ...
  var options = {
    ...
    mosca: {
      ...
      trail: {} // place for clients to note they've seen the message
    }
  };
  ...
  that.server.ascoltatore.publish(
    ...
    function() {
      ...
      that.server.storePacket(packet);
...
Client.prototype.onNonDisconnectClose = function() {
  ...
  if (that.will) {
    ...
    that.server.ascoltatore.publish(
      that.will.topic,
      that.will.payload,
      { qos: that.will.qos, clientId: that.id, mosca: { trail: {} } });
  }
