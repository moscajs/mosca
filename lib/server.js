...
Server.prototype.serve = function(client) {
  ...
  var forward = function(topic, payload, options, sub_topic) {
    debug("delivering message on " + topic + " to " + client.id);

    var qos = Math.min(
            (options && options.qos) || 0,
            client.subscriptions[
                    (sub_topic && sub_topic.replace("*", "#")) || topic] || 0);
  ...
  var unsubscribeMapTo = function(topic) {
    return function(cb) {
      that.ascoltatore.unsubscribe(topic.replace("#", "*"), forward, function(err) {
        if (err) {
          cb(err);
          return;
        }
  ...
  client.on("subscribe", function(packet) {
    var granted = packet.subscriptions.map(function(e) {
      ...
      if (client.subscriptions[e.topic] !== undefined) {
        client.subscriptions[e.topic] = e.qos;
      }
      return e.qos;
    });
