...
var mqttOverWebsockets = require("mqtt.js-over-websockets");
  ...
  async.series([
    ...
    function(cb) {
      ...
      if (that.opts.http) {
        ...
        that.expressMiddleware(app);
        that.attachHttpServer(server);
        that.servers.push(server);
        server.listen(that.opts.http.port || 3000, cb);
      } else {
        cb();
      }
    },
    function(cb) {
      ...
      that.logger.info({ port: that.opts.port, securePort: (that.opts.secure || {}).port }, "server started");
...
/**
 * Attach a Mosca server to a http server
 *
 */
Server.prototype.attachHttpServer = function(server) {
  var that = this;
  mqttOverWebsockets.attachServer(server, function(conn) {
    new Client(conn, that);
  });
};

/**
 * Add the middlewere to express
 */
Server.prototype.expressMiddleware = function(app) {
  if (this.opts.http.static) {
    app.use(express.static(this.opts.http.static));
  }
  if (this.opts.http.bundle) {
    app.get('/bundle.js', browserify(['mqtt.js-over-websockets']));
  }
};
